...
<div class="page-header">
    <h1 class="page-title">Edit Deceased Person</h1>
    <p class="page-subtitle">Update the information for {{ $deceased->name }}</p>
</div>
...
<form action="{{ route('client.decease.update', $deceased->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        ...
        <div class="form-grid">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" 
                        ...
                        value="{{ old('full_name', $deceased->name) }}" 
                        required>
                    @error('full_name')
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" 
                        ...
                        value="{{ old('date_of_birth', $deceased->date_of_birth->format('Y-m-d')) }}" 
                        required>
                    @error('date_of_birth')
                        <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                        ...
                        value="{{ old('date_of_death', $deceased->date_of_death->format('Y-m-d')) }}" 
                        required>
                    @error('date_of_death')
                        <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</span>
                    @enderror
            ...
            <div class="form-group">
                ...
                <input type="text" 
                    ...
                    value="{{ old('cause_of_death', $deceased->cause_of_death) }}">
            ...
            <div class="form-group">
                ...
                <!-- @if($deceased->document)
                    <div style="margin-bottom: 0.5rem;">
                        <span style="color: #4a5568; font-size: 0.875rem;">Current Document: </span>
                        <a href="{{ Storage::url($deceased->document) }}" 
                           target="_blank" 
                           style="color: #3b82f6; text-decoration: underline; font-size: 0.875rem;">
                ...
                <small style="color: #718096; font-size: 0.8rem; margin-top: 0.25rem; display: block;">
                    Upload a new document to replace the existing one. Accepted formats: PDF, JPG, PNG (Max: 2MB)
                </small>
                @error('document')
                    <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</span>
                @enderror
        ...
        <div class="form-grid">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" 
                        ...
                        value="{{ old('kin_name', $deceased->nextOfKins->name ?? '') }}" 
                        required>
                    @error('kin_name')
                        <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <select name="kin_relationship" class="form-select @error('kin_relationship') error @enderror" required>
                        ...
                        <option value="spouse" {{ old('kin_relationship', $deceased->nextOfKins->relationship ?? '') == 'spouse' ? 'selected' : '' }}>Spouse</option>
                        <option value="child" {{ old('kin_relationship', $deceased->nextOfKins->relationship ?? '') == 'child' ? 'selected' : '' }}>Child</option>
                        <option value="parent" {{ old('kin_relationship', $deceased->nextOfKins->relationship ?? '') == 'parent' ? 'selected' : '' }}>Parent</option>
                        <option value="sibling" {{ old('kin_relationship', $deceased->nextOfKins->relationship ?? '') == 'sibling' ? 'selected' : '' }}>Sibling</option>
                        <option value="other" {{ old('kin_relationship', $deceased->nextOfKins->relationship ?? '') == 'other' ? 'selected' : '' }}>Other</option>
                    </select>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="tel" 
                        ...
                        value="{{ old('kin_phone', $deceased->nextOfKins->contact_number ?? '') }}" 
                        required>
                    @error('kin_phone')
                        <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="email" 
                        ...
                        value="{{ old('kin_email', $deceased->nextOfKins->email ?? '') }}">
                    @error('kin_email')
                        <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <textarea name="kin_address" 
                    ...
                    required>{{ old('kin_address', $deceased->nextOfKins->address ?? '') }}</textarea>
                @error('kin_address')
                    <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</span>
                @enderror
            </div>
    ...
    <div class="form-actions">
        <button type="submit" class="btn btn-primary">Update Deceased Record</button>
